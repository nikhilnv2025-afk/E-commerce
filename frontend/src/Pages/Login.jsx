...
const Login =()=>{
    ...
    const location = useLocation() // to check certain url and trigger them if needed
...
const handlesubmit = (e) => {
    e.preventDefault();
    dispatch(loginUser({ email, password })).unwrap().catch((err) => {
        if (err && err.message) {
            alert("Login failed: " + err.message);
        } else {
            alert("Login failed: User does not exist or invalid credentials.");
        }
        window.location.reload(); // Refresh the page after a failed login attempt
    });
}
    return (
        <>
        <div className="flex">
            ...
            <div className="flex justify-center mb-6  ">
                <h2 className="text-xl font-medium">Cartify</h2>
            </div>
            ...
            <div className="mb-4">
                ...
                <input type="email" name="" id="" value={email} onChange={(e)=>setEmail(e.target.value)} className="w-full p-2 border rounded" placeholder="Enter Your Email address" />
            </div>
            <div className="mb-4">
...
<input type="password" value={password} onChange={(e)=>setPassword(e.target.value)}  className="w-full p-2 border rounded " placeholder="Enter Your Password"/>
        ...
        {/* Google Button */}
        {/* <div className="mt-4">
                            <button className="w-full flex items-center justify-center border p-2 rounded-lg font-semibold hover:bg-gray-100 transition">
